...
//*---------------------------------------*
const fGetClosest = function closest ( elem, selector ) {
...
fabricator.setActiveItem = function () {
  ...
  var parsedItems = function () {
    ...
    for (var i = fabricator.dom.menuItems.length - 1; i >= 0; i--) {
      ...
      fabricator.dom.menuItems[i].parentNode.classList.remove('f-active');
  ...
  var setActive = function () {
    ...
    fabricator.dom.menuItems[index].parentNode.classList.add('f-active');

    fGetClosest(fabricator.dom.menuItems[index].parentNode, '.f-top-level').classList.add('f-active-top');

    let secondLevel = fGetClosest(fabricator.dom.menuItems[index].parentNode, '.f-second-level');
    let secondLevels = document.querySelectorAll('.f-second-level');

    if (secondLevels) {
      secondLevels.forEach(level => {
        level.classList.remove('f-active-second');
      });
    }
    
    if ( secondLevel && !secondLevel.classList.contains('f-active-second') ) {
      secondLevel.classList.add('f-active-second')
    }
